...
**Unreleased development checkout.** The interactive flow described here is not included in the published `geoptimize@0.10.0` package.

When the CLI is run directly with no arguments, and both standard input and standard output are TTYs, it offers a short guided flow for scanning one target and writing an offline HTML report:
